...
/**
 * Everything a transformer does that does not depend on the tokenizer.
 * <p>
 * An implementation only provides {@link #transformData}, which protects or unprotects the values
 * this class located in the payload, the headers and the query parameters.
 */
public abstract class AbstractEndPointTransformer implements EndPointTransformer {
     ...
     * <p>
     * {@link DocumentContext#read(JsonPath)} only returns a list for indefinite paths (containing a
    ...
    private Context getRightContext(EndPointTransformConfig cfg) {
        return toRightContext(cfg.getRightContextEvidences());
    }

    /**
     * Build a right context out of its evidences.
     *
     * @param evidences the evidences, keyed by name
     * @return the right context to hand over to the tokenizer
     */
    private static Context toRightContext(Map<String, String> evidences) {
    ...
    /**
     * Build a processing context out of its evidences.
     *
     * @param evidences the evidences, keyed by name
     * @return the processing context to hand over to the tokenizer
     */
    private static ProcessingContext toProcessingContext(Map<String, String> evidences) {
    ...
    private ProcessingContext getProcessingContext(EndPointTransformConfig cfg) {
        ProcessingContext processingContext = toProcessingContext(cfg.getProcessingContextEvidences());
     ...
     * <p>
     * Only the payload is transformed: there is neither header nor query parameter to handle here.
     * The action is read from the {@code Action} processing context evidence; unlike an endpoint
     * configuration there is no {@code BEFORE}/{@code AFTER} to fall back on, so a configuration
     * declaring no action leaves every value unsegmented and lets the tokenizer reject it.
     *
    ...
    public String transformJson(String json,
        ...
        Map<String, JsonPathValue> rpsValuesByJsonPath =
                getRPSValuesFromBody(documentContext, attributeConfigs, processingContextEvidences.get("Action"));

        List<RPSValue> flatList = new ArrayList<>();
        rpsValuesByJsonPath.values().forEach(pathValue -> flatList.addAll(pathValue.plan().rpsValues()));

        if (flatList.isEmpty()) {
            LOG.debug("No value to transform, returning the document unchanged.");
            return json;
        }
